...
def generate_email(results: dict) -> str:
    """
    Generate email-friendly HTML for filtered news results.
    ...
    for query_name, articles in results.items():
        email_html.append(f"<h2 style='color: #0066cc;'>{query_name}</h2>")
        email_html.append("<ul>")

        for article in articles:
            # Use render_article_to_email to format each article
            email_html.append(render_article_to_email(article, query_name=query_name))

        email_html.append("</ul>")  # Close list for this query
...
def render_article_to_email(article: dict, query_name: str = None) -> str:
    """
    Render an individual article as email-compatible HTML.
    ...
    --------
    str
        A formatted email-compatible HTML string for the article.
    """
    html = []
    ...
    # Start the article block
    html.append("<tr>")

    # Add title with link
    html.append(
        f"  <td><strong>[{status}]</strong> <a href='{url}' style='text-decoration: none; color: #0066cc;'>{title}</a>{query_display}</td>")
    html.append("</tr>")

    # Add content summary in text form
    html.append("<tr>")
    html.append(
        f"  <td style='padding: 10px 0;'>{content[:200]}... <a href='{url}' style='color: #0066cc;'>Read more</a></td>")
    html.append("</tr>")
    ...
    if article.get("most_relevant_image"):
        ...
        html.append("<tr>")
        html.append(
            f"  <td><img src='{image_url}' alt='' style='width: 100%; max-width: 400px; height: auto; margin-top: 10px;'></td>")
        html.append("</tr>")

    # Extra padding between articles
